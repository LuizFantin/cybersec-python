...
def register():
    ...
    cipher_mode = input("Enter the cipher mode (CBC, CTR or RSA): ").upper()
    ...
    if cipher_mode == 'CBC':
        ...
        f = open(email + '_key.pem', 'wb')
        f.write(key.export_key('PEM', password))
        f.close()
        pubkey = key.publickey()
        rsa_cipher = PKCS1_OAEP.new(pubkey)
        ciphertext = rsa_cipher.encrypt(password.encode())
    ...
    with open(CREDENTIALS_FILE, "a") as file:
        if cipher_mode == 'RSA':
            file.write(
                f"{email}:{b64encode(salt).decode()}:{pubkey}:{b64encode(ciphertext).decode()}:{str(user_id)}:{cipher_mode}\n"
            )
        else:
            file.write(
...
def login():
    ...
    with open(CREDENTIALS_FILE, "r") as file:
        for line in file:
            stored_email, stored_salt, stored_iv_or_pubkey, stored_ciphertext, user_id, cipher_mode = line.strip().split(":")
            if email == stored_email:
                salt = b64decode(stored_salt)
                iv = b64decode(stored_iv_or_pubkey)
    ...
    if cipher_mode == 'CBC':
        ...
        try:
            with open(email + "_key.pem", "rb") as file:
                privatekey = RSA.import_key(file.read(), password)
                cipher = PKCS1_OAEP.new(privatekey)
                plaintext = cipher.decrypt(ciphertext).decode()
        except FileNotFoundError:
            print("Check if your key exists!")
        return None
